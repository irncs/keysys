-- =============================================
--   ADDICTS.LOL - FULL MERGED SCRIPT
-- =============================================

local script_key = script_key or "NO_KEY"
...
local success, response = pcall(function()
    return game:HttpGet("https://roblox-key-system--ironicfigures.replit.app" .. HttpService:UrlEncode(script_key) .. "&hwid=" .. HttpService:UrlEncode(HWID))
end)
...
shared.Accuracy = {
    ...
    ['Visual Awareness'] = {
        ['Enabled'] = true,
        ['Color'] = Color3.fromRGB(255, 255, 255),
        ['Target Color'] = Color3.fromRGB(255,140,0),
    },
    ['Super Jump'] = {
        ['Enabled'] = true,
        ['Power'] = 265,
...
local function addESPToPlayer(player)
    ...
    esp.nameTag.OutlineColor = Color3.fromRGB(255,140,0)
...
-- UI (Fixed - Smart Position + Camlock Status)
...
text.Position = UDim2.new(0.5, 0, 1, -70)
text.Size = UDim2.new(0, 280, 0, 160)
...
RunService.RenderStepped:Connect(function()
    ...
    table.insert(lines, '<b><font color="rgb(255,140,0)">addicts.lol</font></b>')

    local anythingEnabled = false

    if SpeedEnabled and Config['Speed']['Enabled'] then
        table.insert(lines, '<font color="rgb(255,255,255)">speed-walk</font><font color="rgb(255,140,0)"> {ON}</font>')
        anythingEnabled = true
    end

    if Config['Silent Aim']['Enabled'] and currentTarget then
        local character = currentTarget:FindFirstAncestorWhichIsA("Model")
        local targetPlayer = character and Players:GetPlayerFromCharacter(character)
        local displayName = targetPlayer and targetPlayer.DisplayName or (character and character.Name or "Target")
        table.insert(lines, '<font color="rgb(255,255,255)">silent</font> <font color="rgb(255,140,0)">{' .. displayName .. '}</font>')
        anythingEnabled = true
    end

    if Config['Camera Lock']['Enabled'] and isLocking then
        table.insert(lines, '<font color="rgb(255,255,255)">camlock</font><font color="rgb(255,140,0)"> {ON}</font>')
        anythingEnabled = true
    end

    if infRangeActive and Config['Infinite Range']['Enabled'] then
        table.insert(lines, '<font color="rgb(255,255,255)">inf-range</font><font color="rgb(255,140,0)"> {ON}</font>')
        anythingEnabled = true
    end

    if Config['Visual Awareness']['Enabled'] then
        table.insert(lines, '<font color="rgb(255,255,255)">esp</font><font color="rgb(255,140,0)"> {ON}</font>')
        anythingEnabled = true
    end

    if triggerEnabled and Config['Trigger Bot']['Enabled'] then
        table.insert(lines, '<font color="rgb(255,255,255)">trigger-bot</font><font color="rgb(255,140,0)"> {ON}</font>')
        anythingEnabled = true
    end

    local holdingSuperJump = UserInputService:IsKeyDown(Enum.KeyCode[Config['Keybinds']['Super Jump']])
    if Config['Super Jump']['Enabled'] and holdingSuperJump then
        table.insert(lines, '<font color="rgb(255,255,255)">super-jump</font><font color="rgb(255,140,0)"> {ON}</font>')
        anythingEnabled = true
    end

    if anythingEnabled then
        text.Position = UDim2.new(0.5, 0, 1, -140)
    else
        text.Position = UDim2.new(0.5, 0, 1, -70)
    end
...
print("addicts.lol LOADED ----- ENJOY G!")
